...
Similarly, where 
 ...
 All scripts to reproduce experiments mentioned in our paper are under `experiments/`, where the folder `experiments/model_train` contains the script to train models, and other folters contain scripts for model evaluations (the default pretrained models under `pretrained_models/DeltaSplice_models` are used).